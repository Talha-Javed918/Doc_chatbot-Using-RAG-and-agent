# #fast api key the ka hmne the base bnani hai or is ke endpoints btane hai
...
#     handlers=[
#         logging.FileHandler("chatbot_logs.log"),   # save logs to file
#         logging.StreamHandler()                    # also print in console
#     ]
# )
...
# # load FAISS db from the disk
# faiss_db=FAISS.load_local("faiss_vector_db",embeddings,allow_dangerous_deserialization=True)

# #create the retriever
# retriever=faiss_db.as_retriever(search_kwargs={"k":8})
# logger.info("Faiss database chat model...")
...
# logger.info("chat Model is ready..")
...
#     - "Explain the process described" → "process explanation"
#     - "What is said about financing rules?" → "financing rules"
#     - "Tell me about the dataset" → "dataset information"
...
#         rewritten = question  # fallback to original question
...
#     logger.info(f"Recieved intent:{intent}")
#     # 2. Greeting
#     if intent == "greeting":
#         logger.info("responding with greeting message.")
#         return {
#             "answer": "Hello! 👋 I am your Documents Chatbot. How can I help you?",
#             "total_documents": 0
#         }
...
#         logger.info(f"write query:{search_query}")
#         # Retrieve docs (dynamic, no assumptions)
#         docs = retriever._get_relevant_documents(search_query,run_manager=None)
#         logger.info(f"Retieved {len(docs)}document.")
...
#     # 4. Other
#     logger.warning("No relevant intent matched. Responding with fallback.")
#     return {
#         "answer": "I could not find the answer in the provided Documents.",
#         "total_documents": 0
#     }
...
# #uvicorn main.py:app --reload used that command to run it app:FASTAPI-> instance ,--reload -> automaticaly reload when code change 
# # print("y")
# # print(dir(retriever))

# # storage / id / history.json
# # this is a ine way flow 
